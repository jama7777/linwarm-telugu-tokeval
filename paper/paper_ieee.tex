...
\title{Telugu-First TokEval: Small Dedicated Tokenizers Beat Massive\\ Multilingual Vocabularies for Phone-Scale LLMs}

\author{\IEEEauthorblockN{Gottipati Jamadagni}
\IEEEauthorblockA{\textit{Linwarm} \\ \textit{Hyderabad, India}\\
\texttt{jama@unnanu.com}}
\and
...
\begin{table}[t]
\centering
\caption{Training configurations. *Corpus size measured on disk; training time on M1 Pro laptop.}
\label{tab:train}
\begin{tabular}{lccc}
\toprule
...
\paragraph{Failure modes (TokEval)} All SentencePiece models split ``12345'' into 2--3 pieces (16K: \texttt{▁12, 3, 45}; 32K: \texttt{▁123, 45}), violating digit place-value alignment that TokEval links to math reasoning. Cause: \texttt{split\_by\_number=true}. UTF-8: \texttt{byte\_fallback=false} risks \texttt{<unk>} on emojis; enable for phone robustness.
...
\begin{table}[t]
\centering
\caption{Compression and byte efficiency on 2K Telugu wiki sample. Higher chars/token and bytes/token = better channel use.}
...
\paragraph{Script and digit insights} Telugu script has 60+ base characters plus complex conjuncts (e.g., ``ksha'', ``tra''); English-centric BPE fragments these into bytes, while Unigram's likelihood objective keeps conjuncts intact. Digit test (``12345'' $\rightarrow$ 2--3 pieces) violates TokEval's place-value integrity; we confirmed \texttt{split\_by\_number=true} causes this, and disabling it merges ``12345'' into one piece at cost of +0.02 fertility on Telugu (ablation, not in Table). UTF-8 integrity: with \texttt{byte\_fallback=false}, rare emojis produce \texttt{<unk>} (0.03\% of wiki lines); enabling fallback fixes at cost of +0.01 fertility.
